                ...
                <table class="table table-striped table-condensed">
                    <tbody>
                        ...
                        foreach ($rows as $row) {
                            ...
                            echo '<tr><td colspan="2" class="no-border">' . $r . '. &nbsp;&nbsp;' . ($row->product_name) . ($row->variant ? ' (' . $row->variant . ')' : '') . '<span class="pull-right">' . ($row->tax_code ? '*'.$row->tax_code : '') . '</span></td></tr>';
                            echo '<tr><td class="no-border border-bottom" style="margin-left:1cm"><span style="margin-left:1 cm;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' . $this->sma->formatQuantity($row->quantity) . ' x '.$this->sma->formatMoney($row->net_unit_price + ($row->item_tax / $row->quantity)).'</span></td><td class="no-border border-bottom text-right">' . $this->sma->formatMoney($row->subtotal) . '</td></tr>';
