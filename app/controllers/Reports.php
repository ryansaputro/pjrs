...
class Reports extends MY_Controller
{
    ...
    function get_deposits($company_id = NULL)
    {
     ...
     public function custom_sales($warehouse_id = NULL)
    {
        $this->sma->checkPermissions('products');
        ...
        $this->data['bscustom'] = $this->reports_model->getBestSellerCustomDateWithPagination($startDate, $endDate, $warehouse_id);
        // $this->sma->print_arrays($this->data['m1bs'], $this->data['m2bs'], $this->data['m3bs'], $this->data['m4bs']);
        $this->data['warehouses'] = $this->site->getAllWarehouses();
        $this->data['warehouse'] = $warehouse_id ? $this->site->getWarehouseByID($warehouse_id) : NULL;
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('report_custom_sales')));
        $meta = array('page_title' => lang('report_custom_sales'), 'bc' => $bc);
        $this->page_construct('reports/report_custom_sales', $meta, $this->data);

    }

     public function getProductSales($warehouse_id = NULL)
    {
        $this->sma->checkPermissions('report_custom_sales', TRUE);
        if (!$this->Owner && !$warehouse_id) {
            $user = $this->site->getUser();
            $warehouse_id = $user->warehouse_id;
        }
            ...
            // DataTables old params
            $sEcho   = intval($this->input->get("sEcho"));
            $start   = intval($this->input->get("iDisplayStart"));
            $length  = intval($this->input->get("iDisplayLength"));
            $sSearch = $this->input->get("sSearch"); // global search
            $iSortCol = $this->input->get("iSortCol_0"); // kolom yang di-sort
            $sSortDir = $this->input->get("sSortDir_0"); // arah sort asc/desc

            $columns = ["product_code", "product_name", "total"];
            ...
            $this->db
                ->select(
                    ...
                    'COUNT(' . $this->db->dbprefix('sale_items') . '.id) as total '
                , FALSE)
                ->from($this->db->dbprefix('sale_items'))
                ->join($this->db->dbprefix('sales'), 'sales.id = sale_items.sale_id', 'inner')
                ->join($this->db->dbprefix('products'), 'products.id = sale_items.product_id', 'inner')
                ->where('sales.date >=', $start_date)
                ->where('sales.date <=', $end_date);

            if ($warehouse_id) {
                $this->db->join(
                    "(SELECT product_id FROM {$this->db->dbprefix('warehouses_products')} WHERE warehouse_id = {$warehouse_id}) as wp",
                    'products.id = wp.product_id',
                    'inner'
                );
            }
            ...
            if (!empty($sSearch)) {
                $this->db->group_start();
                $this->db->like('products.name', $sSearch);
                $this->db->or_like('products.code', $sSearch);
                $this->db->group_end();
            }
            ...
            //  HITUNG TOTAL DATA
            // =======================
            $this->db->group_by('products.id, products.name, products.code');
            $subQuery = clone $this->db;
            $totalRecords = $subQuery->count_all_results('', false);

            // =======================
            //  SORTING
            // =======================
            if ($iSortCol !== null && isset($columns[$iSortCol])) {
                $this->db->order_by($columns[$iSortCol], $sSortDir ?: 'asc');
            } else {
            ...
            //  LIMIT + OFFSET
            // =======================
            if ($length != -1) { // -1 artinya tampilkan semua
            ...
            // =======================
            //  FORMAT OUTPUT (sesuai DataTables 1.9.x)
            // =======================
            $output = [
                "sEcho"                => $sEcho,
                "iTotalRecords"        => $totalRecords,   // total data sebelum filter
                "iTotalDisplayRecords" => $totalRecords,   // total data setelah filter
                "aaData"               => $data
            ];
    ...
    public function exportProductSales($type = 'pdf')
    {
        $start_date   = $this->input->get('start_date');
        $end_date     = $this->input->get('end_date');
        $warehouse_id = $this->input->get('warehouse_id');

        // === Query ambil data tanpa pagination ===
        $this->db
            ->select(
                    $this->db->dbprefix('products') . '.name as product_name, ' .
                    $this->db->dbprefix('products') . '.code as product_code, ' .
        ...
        $this->db->group_by('products.id, products.code, products.name')->order_by('products.name', 'ASC');
        ...
        if ($type === 'pdf') {
            ...
            $mpdf->SetHeader('Laporan Produk Terjual||{PAGENO}');
            $mpdf->SetFooter('Generated by Sistem PJRS || {DATE j-m-Y}');
            ...
            $html .= '<table>
                        <thead>
                            ...
                            <th style="width:55%;">Nama Produk</th>
                                <th style="width:25%;">Total Terjual</th>
                            </tr>
                        </thead>
                        <tbody>';
            $no =1;
            ...
            // Tulis ke PDF & download
            $mpdf->WriteHTML($html);
            $filename = 'laporan_produk_'.$start_date.'_'.$end_date.'.pdf';
            ...
            // --- Judul Laporan ---
            $sheet->mergeCells('A1:D1'); // gabung kolom A-C
            ...
            // --- Header Tabel ---
            $headers = ['No', 'Kode', 'Nama Produk', 'Total Terjual'];
            $sheet->fromArray($headers, null, 'A4'); // mulai di row 4

            // Style header
            $headerStyle = $sheet->getStyle('A4:D4');
            $headerStyle->getFont()->setBold(true);
            $headerStyle->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
                        ->getStartColor()->setARGB('FFCCCCCC'); // abu header
            ...
            foreach ($data as $row) {
                ...
                $sheet->setCellValue('B'.$rowNum, $row['product_code']);
                $sheet->setCellValue('C'.$rowNum, $row['product_name']);
                $sheet->setCellValue('D'.$rowNum, $row['total']);
                $rowNum++;
                $no++;
            }

            // --- Style semua data border & alignment ---
            $dataStyle = $sheet->getStyle('A5:D'.($rowNum-1));
