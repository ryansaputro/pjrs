...
class Reports_model extends CI_Model
{
    ...
    /**
     * @getBestSellerCustomDateWithPagination
     * Get best selling products within a custom date range with pagination
     ...
     */
    public function getBestSellerCustomDateWithPagination($start_date, $end_date, $warehouse_id = NULL, $limit = 2, $offset = 0)
    {
        $this->db
            ->select("product_name, product_code")->select_sum('quantity')
            ->join('sales', 'sales.id = sale_items.sale_id', 'left')
            ->where('date >=', $start_date)->where('date <=', $end_date)
            ->group_by('product_name, product_code')->order_by('sum(quantity)', 'desc');
        if ($warehouse_id) {
            $this->db->where('sale_items.warehouse_id', $warehouse_id);
        }
