...
class Companies_model extends CI_Model
{
    ...
    {
        $this->db->select("id, (CASE WHEN company = '-' THEN name ELSE CONCAT(name, ' (', vat_no, ')') END) as text", FALSE);
        $this->db->where(" (id LIKE '%" . $term . "%' OR name LIKE '%" . $term . "%' OR company LIKE '%" . $term . "%' OR vat_no LIKE '%" . $term . "%' OR phone LIKE '%" . $term . "%') ");
