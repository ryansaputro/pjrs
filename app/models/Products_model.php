...
class Products_model extends CI_Model
{
    ...
    public function ClaimPointAdjustment($data, $products)
    {
        if ($this->db->insert('sma_claim_point', $data)) {
            $adjustment_id = $this->db->insert_id();
            foreach ($products as $product) {
                $product['adjustment_id'] = $adjustment_id;
                $this->db->insert('sma_claim_point_items', $product);
                $this->syncAdjustment($product);
            }
            if ($this->site->getReference('qa') == $data['reference_no']) {
                $this->site->updateReference('qa');
    ...
    public function adjustPoint($cust, $val){
        $getCust = $this->db->get_where('companies', array('id' => $cust))->result();
        $currPoint = "";
        foreach ($getCust as $p) {
            $currPoint = $p->award_points;
        }
        $updatePoint = $currPoint - $val;
        $data = array(
             "award_points" => $updatePoint,   
        );
        $this->db->update('companies', $data, array('id' => $cust));
    }
